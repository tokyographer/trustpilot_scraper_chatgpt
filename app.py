...
# Input for Trustpilot URL
trustpilot_url = st.text_input("Enter the Trustpilot URL to scrape reviews", "")

# If a URL is entered, start scraping reviews
if trustpilot_url:
    st.write("Scraping reviews... Please wait.")
    reviews = scrape_reviews(trustpilot_url)

    if reviews:
        # Move the download button above the reviews
        st.subheader("Download Reviews as JSON")
        json_data = json.dumps(reviews, indent=4)  # Convert reviews to JSON format
        st.download_button(
            label="Download reviews as JSON",
            data=json_data,
            file_name="trustpilot_reviews.json",
